...
MongoClient.connect('mongodb://localhost/snippets', function(err, db) {
  ...
  var update = function(name, content) {
    ...
    collection.findAndModify(query, null, update, function(err, object) {
      var snippet = object.value;
      if (!snippet || err) {
        console.error('Could not update object', name);
        db.close();
        return;
      }
  ...
  var del = function(name, content) {
    ...
    collection.findAndRemove(query, function(err, object) {
        var snippet = object.value;
        if (!snippet || err) {
